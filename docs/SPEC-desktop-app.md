...
| 4 | **Pull model** | Download `qwen2.5-coder:7b`. Show progress bar with download percentage. | Run `ollama pull qwen2.5-coder:7b`, parse progress lines from stdout |
...
| **Base model available** | `GET http://localhost:11434/api/tags`, check for `qwen2.5-coder:7b` | Present | Missing -- offer "Pull model" button |
| **Daemon model available** | Same endpoint, check for `daemon` | Present | Missing -- offer "Create model" button |
| **Model loaded in memory** | `GET http://localhost:11434/api/ps` | `daemon` or `qwen2.5-coder:7b` listed | Not loaded (normal if idle > 5 min) |
...
| 5 | **Configure Daemon model** | Set `ollama/daemon` (or `ollama/qwen2.5-coder:7b`) as the default model in `~/.openclaw/openclaw.json`. Write the explicit provider config with `contextWindow: 32768` and `maxTokens: 8192`. |
...
| **Model selector** | Dropdown to switch between available models (e.g., `daemon`, `qwen2.5-coder:7b`). Populated from `GET /api/tags`. |
