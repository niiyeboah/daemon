...
| 4 | **Pull model** | Download the base model. Show progress bar with download percentage. | Run `ollama pull daemon`, parse progress lines from stdout |
...
| **Base model available** | `GET http://localhost:11434/api/tags`, check for `daemon` | Present | Missing -- offer "Pull model" button |
| **Daemon model available** | Same endpoint, check for `daemon` | Present | Missing -- offer "Create model" button |
| **Model loaded in memory** | `GET http://localhost:11434/api/ps` | `daemon` listed | Not loaded (normal if idle > 5 min) |
...
| 5 | **Configure Daemon model** | Set `ollama/daemon` as the default model in `~/.openclaw/openclaw.json`. Write the explicit provider config with `contextWindow: 32768` and `maxTokens: 8192`. |
...
| **Model selector** | Dropdown to switch between available models (e.g., `daemon`). Populated from `GET /api/tags`. |
